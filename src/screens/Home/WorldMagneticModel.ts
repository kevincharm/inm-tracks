/// From: http://www.bdcc.co.uk/Gmaps/WorldMagneticModel.js
...
export default class WorldMagneticModel {
    private coff: string[]
    private c: any[]
    private cd: any[]
    private tc: any[]
    ...
    constructor() {
        ...
        var maxdeg = 12
        var maxord
        var i: number, j: number, D1, D2, n, m
        ...
        for (n = 1; n <= maxord; n++) {
            ...
            */
                ...
                if (n == m) {
                    ...
                    this.dp[m][n] =
                        st * this.dp[m - 1][n - 1] + ct * this.snorm[n - 1 + (m - 1) * 13]
                } else if (n == 1 && m == 0) {
                    ...
                    this.snorm[n + m * 13] =
                        ct * this.snorm[n - 1 + m * 13] - this.k[m][n] * this.snorm[n - 2 + m * 13]
                    this.dp[m][n] =
                        ct * this.dp[m][n - 1] -
                        st * this.snorm[n - 1 + m * 13] -
            ...
            */
                if (st == 0.0 && m == 1) {
                    ...
                    else this.pp[n] = ct * this.pp[n - 1] - this.k[m][n] * this.pp[n - 2]
                    parp = ar * this.pp[n]
    ...
    */
    ...
    knownAnswerTest = () => {
        ...
        return (maxErr * 100) / (maxExp as number) //max % error
    }
}
